	...
	float *C, int ldc)
{
    ...
    for (i = 0; i < M; ++i) {
        for (k = 0; k < K; ++k) {
            float A_PART = ALPHA*A[i*lda + k];
            float b[8], c[8], result[8] = {0,};
            for (j = 0; j < N - 8; j += 8) {
                memcpy(b, &B[k*ldb + j], 32);
                memcpy(c, &C[i*ldc + j], 32);
                result[0] = A_PART * b[0] + c[0];
                result[1] = A_PART * b[1] + c[1];
                result[2] = A_PART * b[2] + c[2];
                result[3] = A_PART * b[3] + c[3];
                result[4] = A_PART * b[4] + c[4];
                result[5] = A_PART * b[5] + c[5];
                result[6] = A_PART * b[6] + c[6];
                result[7] = A_PART * b[7] + c[7];
                memcpy(&C[i*ldc + j], result, 32);
            }
            int prev_end = (N % 8 == 0) ? (N - 8) : (N / 8) * 8;
            for (j = prev_end; j < N; ++j)
                C[i*ldc + j] += A_PART*B[k*ldb + j];
